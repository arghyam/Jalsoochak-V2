...
import com.example.user.clients.KeycloakClient;
...
import com.example.user.repository.InviteTokenRow;
import com.example.user.repository.TenantUserRecord;
import com.example.user.repository.UserCommonRepository;
import com.example.user.repository.UserTenantRepository;
...
//                    if (headerRow == null) {
//                        throw new com.example.user.exceptions.BadRequestException("Header row is missing");
//                    }
//
//                    for (int i = 0; i < EXPECTED_HEADERS.length; i++) {
//                        String cellValue = getCellValue(headerRow.getCell(i)).toLowerCase().trim();
//                        if (!EXPECTED_HEADERS[i].equals(cellValue)) {
//                            throw new com.example.user.exceptions.BadRequestException("Invalid header at column " + (i + 1)
...
//                    if (headerRow == null) {
//                        throw new com.example.user.exceptions.BadRequestException("CSV file is empty or missing header row");
//                    }
//
//                    for (int i = 0; i < EXPECTED_HEADERS.length; i++) {
//                        String cellValue = i < headerRow.length ? headerRow[i].toLowerCase().trim() : "";
//                        if (!EXPECTED_HEADERS[i].equals(cellValue)) {
//                            throw new com.example.user.exceptions.BadRequestException("Invalid CSV header at column " + (i + 1)
//                                    + ": expected '" + EXPECTED_HEADERS[i] + "', found '" + cellValue + "'");
//                        }
//                    }
//
...
//
//            if (!errors.isEmpty()) {
//                throw new com.example.user.exceptions.BadRequestException("Validation failed", errors);
//            }
//
